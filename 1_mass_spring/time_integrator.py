...
def step_forward(x, e, v, m, l2, k, h, tol):
    ...
    inputs = []
    outputs = []
    array, input, p = search_dir(x, e, x_tilde, m, l2, k, h)
    arrays.append(array)
    inputs.append(input)
    outputs.append(p)
    ...
    return x, v, arrays, inputs, outputs


def IP_val(x, e, x_tilde, m, l2, k, h):
    return InertiaEnergy.val(x, x_tilde, m) + h * h * MassSpringEnergy.val(
        x, e, l2, k
    )  # implicit Euler


def IP_grad(x, e, x_tilde, m, l2, k, h):
    return InertiaEnergy.grad(x, x_tilde, m) + h * h * MassSpringEnergy.grad(
        x, e, l2, k
    )  # implicit Euler
...
def IP_hess(x, e, x_tilde, m, l2, k, h):
    ...
    H = sparse.coo_matrix(
        (IJV[2], (IJV[0], IJV[1])), shape=(len(x) * 2, len(x) * 2)
    ).tocsr()
    return H
...
def search_dir(x, e, x_tilde, m, l2, k, h):
    ...
    return (
        projected_hess.todense(),
        -reshaped_grad,
        spsolve(projected_hess, -reshaped_grad).reshape(len(x), 2),
    )
